# File generated from our OpenAPI spec and handwritten Terraform acceptance templates.
# Handwritten Terraform acceptance source emitted by sdk-codegen.
resource "stripe_customer" "customer" {
  name  = "acctest-subscription-schedule-start-customer-{{RAND}}"
  email = "sdk-codegen+subsched-start-{{RAND}}@example.com"
}

resource "stripe_product" "product" {
  name = "acctest-subscription-schedule-start-product-{{RAND}}"
}

resource "stripe_price" "price" {
  product     = stripe_product.product.id
  currency    = "usd"
  unit_amount = 1750

  recurring {
    interval = "month"
  }
}

resource "stripe_subscription_schedule" "test" {
  customer     = stripe_customer.customer.id
  start_date   = 1893888000
  end_behavior = "release"

  metadata = {
    suite = "sdk-codegen"
    case  = "subscription_schedule_start_date_regression"
  }

  phases = [
    {
      iterations = 1
      items = [
        {
          price    = stripe_price.price.id
          quantity = 1
        },
      ]
    },
  ]
}
